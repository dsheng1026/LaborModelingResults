...
df_crop <- do.call(rbind, df_list) %>%
  CLUSTER_CROP() %>%
  CLUSTER_CROP2() %>%
  dplyr::filter(!group %in% c("Forest", "Pasture", "Biomass"))
...
Figure <- df_crop %>%
  dplyr::group_by(scenario, year, region, group) %>%
  dplyr::summarise(value = sum(value)) %>%
  dplyr::filter(year >= 2015,
                group != "Biomass") %>%
  group_by(scenario, region, group) %>%
...
Figure <- df_crop %>%
  ...
  mutate(UC_L = value * Labor / Q) %>% # $/kg
  group_by(scenario, group, region) %>%
  mutate(index = UC_L / UC_L[year == 2015])

# Q-weighted average labor yld
MEAN <- Figure %>%
  group_by(year, region) %>%
  summarise(Exp_L = sum(value * Labor),
            Q = sum(Q)) %>%
  group_by(region) %>%
  mutate(UC_L = Exp_L / Q,
